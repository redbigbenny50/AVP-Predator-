 ...
 *
 * <p>The Sodium check previously routed through Architectury's
 * {@code dev.architectury.platform.Platform}, which is not a declared or
 * shipped dependency of this project — causing a {@link NoClassDefFoundError}
 * at runtime the first time a predator vision post effect evaluated
 * {@code shouldRun()}. The check is now performed reflectively against the
 * active mod loader (Fabric Loader or NeoForge), with no compile-time
 * dependency on either, and the result is cached so the per-frame render
 * path never pays the reflection cost more than once.</p>
 */
public final class PredatorClientCompatibility {
     ...
     *
     * <p>Tries Fabric Loader first, then NeoForge's mod list. Both lookups are
     * reflective, so this class has no compile-time dependency on either
     * loader and can never trigger a {@link NoClassDefFoundError} when the
     * other loader is absent. Any failure is treated as "not loaded".</p>
     */
